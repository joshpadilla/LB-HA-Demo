...
data "template_file" "Bird-Conf" {
 ...
 vars = {
    Elastic_IP="${metal_reserved_ip_block.ips.cidr_notation}"
    Server_Private_IPv4="${metal_device.LBs.*.access_private_ipv4[count.index]}"
 }
}
...
resource "metal_port_vlan_attachment" "LBs-attach" {
  count     = "${var.loadbalancer_count}"
  device_id = "${metal_device.LBs.*.id[count.index]}"
  port_name = "eth1"
  vlan_vnid = "${metal_vlan.vlan1.vxlan}"
}

# Enable BGP on each LB node
resource "metal_bgp_session" "bgp_test_ipv4" {
  count          = "${var.loadbalancer_count}"
  device_id      = "${metal_device.LBs.*.id[count.index]}"
  address_family = "ipv4"
}
...
resource "null_resource" "setup_node" {
  ...
  depends_on = [metal_port_vlan_attachment.webservers-attach]
  ...
  connection {
    type = "ssh"
    user = "root"
    host = "${element(metal_device.LBs.*.access_public_ipv4, count.index)}"
...
data "template_file" "keepalived-master" {
 ...
 vars = {
    Elastic_IP = "${metal_reserved_ip_block.ips.cidr_notation}"
    Main_Server_Private_IPv4 = "${metal_device.LBs[0].access_private_ipv4}"
    Peer_Server_Private_IPv4 = "${metal_device.LBs[1].access_private_ipv4}"
    Role = "MASTER"
    Priority = "101"
 }
}
...
resource "null_resource" "setup_keepalived_master" {
   ...
   connection {
    type = "ssh"
    user = "root"
    host = "${metal_device.LBs[0].access_public_ipv4}"
    private_key = "${file("~/.ssh/id_rsa")}"
   }
...
data "template_file" "keepalived-backup" {
 ...
 vars = {
    Elastic_IP = "${metal_reserved_ip_block.ips.cidr_notation}"
    Main_Server_Private_IPv4 = "${metal_device.LBs[1].access_private_ipv4}"
    Peer_Server_Private_IPv4 = "${metal_device.LBs[0].access_private_ipv4}"
    Role = "BACKUP"
    Priority = "100"
 }
}
...
resource "null_resource" "setup_keepalived_backup" {
   ...
   connection {
    type = "ssh"
    user = "root"
    host = "${metal_device.LBs[1].access_public_ipv4}"
    private_key = "${file("~/.ssh/id_rsa")}"
   }
